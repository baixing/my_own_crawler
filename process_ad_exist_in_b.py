...
def check_ad_exist(ad_id):
    """检查广告是否存在"""
    url = f"https://api.baixing.com.cn/v2/archive/{ad_id}"
    try:
        response = requests.get(url)
        data = response.json()
        return ad_id, 'yes_b' if data.get('code') == 200 else 'no'
...
def process_pending_ads(batch_size=50, max_workers=10):
    ...
    try:
        ...
        # 获取需要处理的行
        pending_mask = (df['ad_isexist'] == 'no') & (df['ad_isexist_inb']== 'unkown') & (df['adid'].notna()) & (df['adid'] != '')
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            for i in range(0, len(ad_ids), batch_size):
                ...
                for ad_id, result in results.items():
                    ...
                    print(ad_id,result)
                    if result == 'no':
                        df.at[idx, 'expect_code'] = '404'
                    if result == 'yes_b':
                        df.at[idx, 'expect_code'] = '200'
                    df.at[idx, 'ad_isexist_inb'] = result
